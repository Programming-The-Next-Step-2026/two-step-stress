...
def build_fixation(win: visual.Window) -> visual.TextStim:
    """Central fixation cross for the inter-trial interval."""
    return _text(win, "+", FIX_HEIGHT, (0.0, 0.0))


def build_stage1(win: visual.Window) -> Stage1Stims:
    """Two colour-coded rockets (left / right) for the Stage-1 choice."""
...
def build_transition(win: visual.Window) -> TransitionStims:
    """A neutral rocket arriving at the colour-coded destination planet."""
...
def build_stage2(win: visual.Window) -> Stage2Stims:
    """Planet-coloured backdrop circle and its two labelled aliens."""
...
def update_treasure_counter(counter: visual.TextStim, total: int) -> None:
    """Update the running treasure total."""
...
def draw_fixation(
    ...
    """Draw the ITI fixation + counter and flip; return the flip timestamp."""
    fixation.draw()
    counter.draw()
    return win.flip()
...
def draw_stage1(
    ...
    """Draw the Stage-1 rockets (+ optional 1-back letter) and flip.
...
def draw_stage2(
    ...
    """Draw the Stage-2 aliens on their planet-coloured backdrop and flip."""
    stims.planet.fillColor = PLANET_COLORS[stage2_state]
    stims.planet.draw()
...
def draw_reward(
    ...
    """Draw reward (green disc + “+1”) or no-reward (grey disc + “0”) and flip."""
    if rewarded:
...
def draw_too_slow(
    ...
    """Draw the 'Too slow!' timeout message and flip."""
    too_slow.draw()
    counter.draw()
    return win.flip()
...
def show_message(win: visual.Window, screen: MessageScreen) -> float:
    """Draw a message screen (welcome / instructions / banner / break / end)."""
    screen.main.draw()
    screen.hint.draw()
    return win.flip()
